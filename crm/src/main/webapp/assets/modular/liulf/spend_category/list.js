layui.use(['layer', 'table', 'ax', 'laydate', 'admin','form'], function () {
    ...
    dataTable.initColumn = function () {
        return [[
            {type: 'checkbox'},
            {field: 'spend_category_id', hide: true, sort: true, title: 'id'},
            {field: 'spend_category_code', sort: true, title: '分类编码'},
    ...
    dataTable.onEdit = function (data) {
        ...
        top.layui.admin.open({
            ...
            title: '编辑支出分类',
            area: ['800px', '440px'],
            content: Feng.ctxPath + '/spend_category/view_update?spend_category_id=' + data.spend_category_id,
            end: function () {
    ...
    dataTable.onDelete = function (data) {
        ...
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/spend_category/delete", function (data) {
                if(data.success){
            ...
            ajax.set("ids", data.spend_category_id);
            ajax.start();
        };
        Feng.confirm("是否作废【" + data.spend_category_name + "】支出分类?", operation);
    };
    ...
    dataTable.search = function () {
        var queryData = {};
        queryData['spend_category_name'] = $("#spend_category_name").val();
        queryData['spend_category_code'] = $("#spend_category_code").val();
    ...
    dataTable.openAdd = function () {
        ...
        top.layui.admin.open({
            ...
            title: '添加支出分类',
            area: ['800px', '440px'],
            content: Feng.ctxPath + '/spend_category/view_add',
            end: function () {
                admin.getTempData('formOk') && table.reload(dataTable.tableId);
            }
        });
    ...
    dataTable.deleteBatch = function () {
        ...
        for (var i = 0; i < checkRows.data.length; i++) {
            ids.push(checkRows.data[i]["spend_category_id"]);
        }

        Feng.confirm("是否作废选中的支出分类?", function () {
            var ajax = new $ax(Feng.ctxPath + "/spend_category/delete", function (data) {
                Feng.success("作废支出分类成功!");
                dataTable.search();
            }, function (data) {
                Feng.error("作废支出分类失败!");
            });
    ...
    var tableResult = table.render({
        ...
        url: Feng.ctxPath + '/spend_category/listdata',
        page: true,
        height: "full-158",
        cellMinWidth: 100,
